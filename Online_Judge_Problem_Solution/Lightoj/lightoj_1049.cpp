...
void dfs(int node)
{
    ...
    if(V[node].size()>0)
    {
        ...
        {
            color[V[node][0]]=false;//printf("if %d %d\n",node,V[node][0]);
            dfs(V[node][0]);
        }
        else if(V[node].size()>1)
        {
            if(color[V[node][1]]==true)
                color[V[node][1]]=false;//printf("if2 %d %d\n",node,V[node][1]);
            dfs(V[node][1]);
        }
        else
        {
            ...
            if(color[x]==true)
            {
                temp=x;
                color[temp]=false;
                ans+=M[make_pair(temp,node)];//printf("else0 %d %d a%d\n",node,V2[node][0],ans);
                dfs(temp);
            }
    ...
    else
    {
        ...
        else if(color[V2[node][1]]==true)
        {
            temp=V2[node][1];
            color[temp]=false;
            ans+=M[make_pair(temp,node)];//printf("else1 %d %d a%d\n",node,V2[node][1],ans);
            dfs(temp);
        }
        else
        {
